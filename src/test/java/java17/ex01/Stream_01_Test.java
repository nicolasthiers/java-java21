...
public class Stream_01_Test {
	...
	public void test_stream_findFirst() throws Exception {
		...
		Optional<Order> result = orders.stream().filter(p -> p.getCustomer().getFirstname().equals("Sophie")).findFirst();
	...
	public void test_stream_filter_adv() throws Exception {
		...
		// TODO pizza Pépéroni
		List<Order> result = orders.stream().filter(commande -> commande.getPizzas().stream().allMatch(pizza -> pizza.getName().equals("Pépéroni"))).toList();
